...
const MermaidDiagram: React.FC<MermaidDiagramProps> = ({ chart }) => {
  ...
  const sanitizeMermaid = (input: string) => {
    ...
    const isValidNode = (node: string): boolean => {
      return /^[a-zA-Z0-9_]+(\[.+\])?$/.test(node.trim());
    };

    // Helper function to clean node ID (remove label part)
    const getNodeId = (node: string): string => {
      const match = node.match(/^([a-zA-Z0-9_]+)/);
    ...
    for (let i = hasHeader ? 1 : 0; i < lines.length; i++) {
      ...
      if (hasArrow) {
        ...
        // Validate connection: must have at least source, arrow, target (3 parts minimum)
        if (sanitizedParts.length >= 3) {
          // Check if first and last parts are valid nodes
        ...
        if (nodeMatch) {
          ...
          const label = nodeMatch[2]
            ...
            .replace(/[^a-zA-Z0-9\s-]/g, '')
            .trim();
          nodeDefinitions.push(`${id}["${label}"]`);
        }
      }
    }

    // Add node definitions first, then connections
    processedLines.push(...nodeDefinitions);
